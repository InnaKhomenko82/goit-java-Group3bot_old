...
public class BlockWithLessons extends GoogleApiToJson {
    ...
    public static void getAll(List<Sheet> sheets, String spreadSheetId, Sheets service) throws IOException {
        for (int j = 0; j < sheets.size(); j++) {
            if (numberBlock == 0) {
                ...
                Sheet sheet = sheets.get(numberBlock);
                GoogleApiToJson.blockName1 = sheet
                        .getProperties()
                        .getTitle();
                GoogleApiToJson.blocksNames.add(GoogleApiToJson.blockName1);// blockName - назание Гугл-Листа
